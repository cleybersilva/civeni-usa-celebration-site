-- Security Enhancement: Layered RLS for event_registrations (Fixed)
...
-- Create function to log registration data access (for modifications only)
...
-- Create triggers to log modifications to sensitive registration data
...
-- Create a function to securely access registration details with logging
...
BEGIN
  ...
  -- Return appropriate data based on user permissions
  ...
  ELSE
    ...
    SELECT json_build_object(
      ...
      'created_at', er.created_at,
      'updated_at', er.updated_at,
      -- Sensitive fields are excluded for non-root admins
...
'Event registration data with layered RLS security: admin_root gets full access, regular admins get limited access to non-sensitive data only. All modifications are logged in registration_access_logs.';
...
'Sensitive payment data view. Only accessible by admin_root users.';
