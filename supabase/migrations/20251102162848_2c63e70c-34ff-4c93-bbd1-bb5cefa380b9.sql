...
BEGIN
  FOR func_signature IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'can_submit_trabalho'
  LOOP
    EXECUTE 'DROP FUNCTION IF EXISTS public.can_submit_trabalho CASCADE';
  END LOOP;
END $$;

-- Remover também o trigger que depende dela
DROP TRIGGER IF EXISTS trg_enforce_submission_limit ON public.submissions;
DROP FUNCTION IF EXISTS public.enforce_submission_limit CASCADE;

-- 3.1 Recriar função can_submit_trabalho com normalização
...
-- 3.2 Recriar trigger de proteção
...
BEGIN
  SELECT * INTO r
  FROM public.can_submit_trabalho(NEW.email, NEW.autor_principal, NEW.tipo, 'civeni-2025');
