...
-- 3. Política de leitura pública para o bucket site-civeni (drop first if exists)
DROP POLICY IF EXISTS "site_civeni_public_read" ON storage.objects;
CREATE POLICY "site_civeni_public_read"
ON storage.objects
FOR SELECT
USING (bucket_id = 'site-civeni');

-- 4. Política de escrita para admins no bucket site-civeni
DROP POLICY IF EXISTS "site_civeni_admin_write" ON storage.objects;
CREATE POLICY "site_civeni_admin_write"
ON storage.objects
FOR INSERT
WITH CHECK (bucket_id = 'site-civeni' AND is_current_user_admin());

DROP POLICY IF EXISTS "site_civeni_admin_update" ON storage.objects;
CREATE POLICY "site_civeni_admin_update"
ON storage.objects
FOR UPDATE
