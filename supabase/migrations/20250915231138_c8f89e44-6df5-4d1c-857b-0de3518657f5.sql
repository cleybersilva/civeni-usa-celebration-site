...
SELECT d.id, start_at, session_type, title, description, modality, order_in_day, is_published
FROM days_data d
CROSS JOIN (
  VALUES
...
SELECT d.id, start_at, session_type, title, description, modality, is_parallel, order_in_day, is_published
FROM day2 d
CROSS JOIN (
  VALUES
