-- Corrigir can_submit_trabalho para usar email_norm ao invés de email_normalized
...
BEGIN
  ...
  -- Verificar inscrição (primeiro por e-mail)
  -- Aceita: paid, succeeded, confirmed, completed
  -- OU pending se tiver stripe_session_id (pagamento iniciado)
  SELECT id INTO v_inscricao_id 
  FROM event_registrations
  WHERE email_normalized = v_email_norm
    AND (
      payment_status IN ('paid', 'succeeded', 'confirmed', 'completed')
      OR (payment_status = 'pending' AND stripe_session_id IS NOT NULL)
    )
  LIMIT 1;
  ...
  -- Contar submissões existentes (usando email_norm e autor_principal_normalized)
