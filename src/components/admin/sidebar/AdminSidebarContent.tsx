...
const AdminSidebarContent = ({ visibleItems, activeTab, onTabChange }: AdminSidebarContentProps) => {
  return (
    <SidebarContent 
      className="px-2 py-3 tablet:py-3 md:py-4 flex-1 overflow-y-auto scrollbar-none"
      style={{ 
        textAlign: 'left', 
        overflowX: 'hidden',
        scrollbarWidth: 'none',
    ...
    >
      <SidebarMenu className="space-y-0.5 tablet:space-y-0.5 md:space-y-1" style={{ textAlign: 'left' }}>
        {visibleItems.map((item) => (
          <SidebarMenuItem key={item.id} style={{ textAlign: 'left' }}>
            <SidebarMenuButton
            ...
            >
              ...
              <span 
                className="transition-all duration-200 truncate text-left" 
