...
const AdminSidebarContent = ({ visibleItems, activeTab, onTabChange }: AdminSidebarContentProps) => {
  return (
    <SidebarContent className="px-2 py-3 md:py-4 flex-1 overflow-y-auto">
        ...
        {visibleItems.map((item) => (
          <SidebarMenuItem key={item.id}>
            <SidebarMenuButton
              ...
              className="w-full justify-start px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-medium transition-all duration-300 hover:bg-civeni-blue/10 hover:text-civeni-blue data-[active=true]:bg-civeni-blue data-[active=true]:text-white data-[active=true]:shadow-md rounded-lg group"
            >
